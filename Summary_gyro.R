...
gyroConvert = function(mdata,tag = "", lowpass=T, gyrpercent = 0.9){
  ...
  for(i in 2:nrow(mdata)){
    ...
    rotationAnglesMag = c(rotationAnglesMag,computeAngle(cuMatrixComplementary[,2],initMatric[,2]))
    rotationAngles = c(rotationAngles,computeAngle(cuMatrixComplementary[,3],initMatric[,3]))
    ...
    carlibratedMatrix = getCarlibratedMatirx(computedInitGV,rotationByMatrix(initGV,initMatric))
    carlibratedGlobalAcc = rotationByMatrix(unCarliratedGlobalAcc, carlibratedMatrix)
    cbAccs = rbind(cbAccs, carlibratedGlobalAcc)
#     
#     rotationAnglesMag = c(rotationAnglesMag,computeAngle(carlibratedMatrix[,2],initMatric[,2]))
#     rotationAngles = c(rotationAngles,computeAngle(carlibratedMatrix[,3],initMatric[,3]))
#     
...
#   
  ...
  plot(rotationAngles,type = "l",main = paste(tag,"angle gravity drift"), 
       ylab = "angle gravity drift (in degree)", cex.main=1.5, cex.lab=1.5)
  plot(rotationAnglesMag,type = "l",main = paste(tag,"angle magnet drift"), 
       ylab = "angle magnet drift (in degree)", cex.main=1.5, cex.lab=1.5)
  write.csv(cbind(rotationAngles, rotationAnglesMag), paste("./summary_gyro/drift_",tag,".csv",sep =""),row.names = F)
}
