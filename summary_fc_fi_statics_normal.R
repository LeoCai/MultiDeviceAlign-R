#普通行走计算水平面偏角
#论文中先用陀螺仪跟踪坐标系
#此处直接用磁力计同步坐标系，但假设初始情况不知道
#计算角度后算与磁力计方向偏角（也就是与向量0,1,0 的夹角）
source("./utils.R")
...
write.csv(fcbar,"./summary_fc_fi/normal_fcbar.csv",row.names = F)
write.csv(fibar,"./summary_fc_fi/normal_fibar.csv",row.names = F)
write.csv(finbar,"./summary_fc_fi/normal_finbar.csv",row.names = F)
